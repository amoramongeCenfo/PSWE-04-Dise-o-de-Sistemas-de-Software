...
| Escenarios de calidad | 6 escenarios (Avance 1) → **7 escenarios** con la adición de QS-07 (Entrega final) | QA-03 (Interoperabilidad) no tenía escenario propio para el riesgo de REST-05 (motor intercambiable). Se añadió QS-07 para medir la sustitución del motor sin modificar el sistema; la justificación de QA-03 en §3.2 se mantiene sin referencia retroactiva a QS-07 porque el escenario no existía en el Avance 1. |
...
| `FiscalRetryEventHandler` | Control | Aplica la política de reintento de envío a Hacienda y su orden FIFO. Consume `IFiscalServiceClient` (implementado por `FiscalHttpClient`) para el reenvío puntual por REST interno (§10.2.2). |
...
> **Nota sobre los CU marcados con MA.** Los 4 casos de uso cuyo componente es MA (CU-CLI-05, CU-MOT-01 a CU-MOT-03) se incluyen en la tabla para completar la trazabilidad de §1.4 — su realización está fuera del sistema por decisión deliberada (REST-05, §3.4). CU-MOT-04 es el único caso de uso del motor que toca el sistema: lo hace exclusivamente a través del handoff de preventa en el Componente 3.
...
|---|---|---|---|
| **Plano de cómputo ↔ plano de datos** (API/Servicio Fiscal ↔ SQL Server) | **CP — se sacrifica disponibilidad** (equivalente a PC en PACELC) | Si la base de datos no responde, la emisión **se rechaza** con `500` y rollback completo. No se emite ningún comprobante "provisional" ni se difiere la escritura a memoria. Prefiere no facturar antes que facturar sin evidencia | §13.2 (Disponibilidad vs. Integridad fiscal) |
...
| **Estado fiscal** | Situación del comprobante ante la autoridad: `Aceptado`, `Rechazado` o el estado interno `PendienteValidacionHacienda` (ver entrada propia de cada estado). Se distingue del estado comercial de la cotización, que sí es reversible y editable. |
